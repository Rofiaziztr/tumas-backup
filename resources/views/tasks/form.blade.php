<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <select class="form-select" id="category" name="category">
            ...
            <option value="Kuis" {{ old('category', $task->category ?? '') == 'Kuis' ? 'selected' : '' }}>Kuis
            </option>
    ...
    @php
        ...
        if (isset($task) && empty($deadlineValue)) {
            $deadlineValue = $task->deadline->format('Y-m-d H:i');
        }
    @endphp

    <div class="col-md-4 mb-3">
        <label for="deadline-picker" class="form-label">Deadline *</label>
        <input type="text" class="form-control" id="deadline-picker" name="deadline" value="{{ $deadlineValue }}"
            required>
    </div>
    ...
    <div class="col-md-4 mb-3">
        ...
        <select class="form-select" id="status" name="status" required>
            <option value="todo" {{ old('status', $task->status ?? '') == 'todo' ? 'selected' : '' }}>Belum
                Dikerjakan</option>
